...
function addUser($username, $firstname, $lastname, $supervisor, $email, $phone, $usertype, $institution, $address, $postcode, $password, $current_facilities,$site_url)
{


	$username = secureInput($username);
	$firstname = secureInput($firstname);
	$lastname = secureInput($lastname);
	$supervisor = secureInput($supervisor);
	$email = secureInput($email);
	$phone = secureInput($phone);
	$usertype = secureInput($usertype);
	$institution = secureInput($institution);
	$address = secureInput($address);
	$postcode = secureInput($postcode);
	$password = secureInput($password);
	$current_facilities = secureInput($current_facilities);
	...
	$sql = "INSERT INTO users (username,password,first_name,last_name,email,supervisor,phone,address,postcode,usertype,institution,current_facilities,active,level_access,act_key,reg_date) VALUES ('".$username."', '".$password."', '".$firstname."', '".$lastname."', '".$email."', '".$supervisor."', '".$phone."', '".$address."', '".$postcode."', '".$usertype."', '".$institution."', '".$current_facilities."',0,2,'".$activation_key."','".$reg_date."')";
	...
	if($res){
		...
		<p>Dear ".$username.", thank you for registering at ".$site_url.".</p>
		...
		if(0){//$mail_send = mail($email, $subject, $message, $headers)) {
		 return 99;
		 }
			return 1;
	}
